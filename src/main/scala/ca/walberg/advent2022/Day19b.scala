...
@main def Day19b(args: String*): Unit = {
  ...
  def geodeMined(b: Blueprint, minutesLeft: Int)(ore: Int, clay: Int, obsidian: Int, geodes: Int)(oreRobots: Int, clayRobots: Int, obsidianRobots: Int, geodeRobots: Int): Int =
    ...
    val enoughOreRobots = oreRobots == Math.max(Math.max(Math.max(b.orePerOreRobot, b.orePerClayRobot), b.orePerObsidianRobot), b.orePerGeodeRobot)
    ...
    Seq(
      ...
      if (!canBuildGeodeRobot && canBuildObsidianRobot) Some(geodeMined(b, minutesLeft - 1)(ore + oreRobots - b.orePerObsidianRobot, clay + clayRobots - b.clayPerObsidianRobot, obsidian + obsidianRobots, geodes + geodeRobots)(oreRobots, clayRobots, obsidianRobots + 1, geodeRobots)) else None,
      if (!canBuildGeodeRobot && !canBuildObsidianRobot && canBuildClayRobot) Some(geodeMined(b, minutesLeft - 1)(ore + oreRobots - b.orePerClayRobot, clay + clayRobots, obsidian + obsidianRobots, geodes + geodeRobots)(oreRobots, clayRobots + 1, obsidianRobots, geodeRobots)) else None,
      if (!canBuildGeodeRobot && !canBuildObsidianRobot && !enoughOreRobots && canBuildOreRobot) Some(geodeMined(b, minutesLeft - 1)(ore + oreRobots - b.orePerOreRobot, clay + clayRobots, obsidian + obsidianRobots, geodes + geodeRobots)(oreRobots + 1, clayRobots, obsidianRobots, geodeRobots)) else None,
