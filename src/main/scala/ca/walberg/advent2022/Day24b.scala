...
@main def Day24b(args: String*): Unit = {
  ...
  def shortestPath(from: (Int, Int), to: (Int, Int), startingMinute: Int): Int =
    val queue = mutable.Queue[Space]()
    val explored = mutable.Queue[Space]()
    queue.append(Space(x = from._1, y = from._2, minutesAway = startingMinute))
    while (queue.nonEmpty) {
      val e = queue.dequeue()
      val valley = valleyForMinute(e.minutesAway + 1)
      if (e.x == to._1 && e.y == to._2) {
      ...
      Seq(
        ...
        if (e.y < valleyHeight - 1 && valley(e.y + 1)(e.x) == '.') Some((e.x, e.y + 1)) else None,
  ...
  val entrance = (1, 0)
  val exit = (valleyWidth - 2, valleyHeight - 1)
  val toExit = shortestPath(entrance, exit, 0)
  val backToEntrance = shortestPath(exit, entrance, toExit)
  val backToExit = shortestPath(entrance, exit, backToEntrance)
  println(s"$toExit -> $backToEntrance -> $backToExit")
}
